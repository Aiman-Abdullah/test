...
app = DjangoDash('dim_color_datatable', external_stylesheets=[dbc.themes.BOOTSTRAP],
                # meta_tags=[
                #             {'name': 'viewport',
...
dim_color_df = Dim_color.objects.all()
dim_color_df = read_frame(dim_color_df)
df = dim_color_df
...
options = []
for color_description in df.color_description:
    options.append({'label': color_description, 'value': color_description})
        ...
        dbc.Col([
                html.Div(
                    children = dash_table.DataTable(
                        ...
                        columns=[
                            dict(id='color_key', format=Format().align(Align.left))
                        ,   dict(id='color_unique_key') # , type='numeric'
                        ,   dict(id='color_table') # , type='numeric'
                        ,   dict(id='color_group')
                        ,   dict(id='color_name')
                        ,   dict(id='color_description')
                        ,   dict(id='color_bo_date')
                        ,   dict(id='color_disc_group') 
                        ,   dict(id='color_formula_group')
                        ,   dict(id='color_begin_date')
                        ,   dict(id='color_discontinued_date')
                        ,   dict(id='color_inactive_date')
                        ,   dict(id='color_constants')
...
@app.callback(
    Output('color_table', 'data'),
    [Input('color_table','active_cell')
...
def update_table(interval, start_date, end_date, value):
    ...
    if value:
        df = df[df['color_description'].isin(value)]
    else:
        pass
