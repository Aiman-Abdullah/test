...
from dim_date.models import Dim_date
...
app = DjangoDash('dim_date_datatable', external_stylesheets=[dbc.themes.BOOTSTRAP],
                # meta_tags=[
                #             {'name': 'viewport',
...
dim_date_df = Dim_date.objects.all()
dim_date_df = read_frame(dim_date_df)
df = dim_date_df
...
options2 = []
for date_name in df.date_date_name:
    options2.append({'label': date_name, 'value': date_name})
        ...
        dbc.Col([
                html.Div(
                    children = dash_table.DataTable(
                        ...
                        columns=[
                            dict(id='date_date_key', format=Format().align(Align.left))
                        ,   dict(id='date_date_name', type='numeric')
                        ,   dict(id='date_full_date_usa')
                        ,   dict(id='date_day_of_week')
                        ,   dict(id='date_day_name')
                        ,   dict(id='date_day_of_month')
                        ,   dict(id='date_day_of_year') 
                        ,   dict(id='date_week_of_year')
                        ,   dict(id='date_month_name')
                        ,   dict(id='date_month_of_year')
                        ,   dict(id='date_quarter')
                        ,   dict(id='date_quarter_name')
                        ,   dict(id='date_year_name')
                        ,   dict(id='date_is_weekday')
...
@app.callback(
    Output('date_table', 'data'),
    [Input('date_table','active_cell')
...
def update_table(interval, start_date, end_date, value):
    ...
    if value:
        df = df[df['date_date_name'].isin(value)]
    else:
        pass
