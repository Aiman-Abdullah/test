...
from dim_product_group.models import Dim_product_group
...
app = DjangoDash('dim_product_group_datatable', external_stylesheets=[dbc.themes.BOOTSTRAP],
                # meta_tags=[
                #             {'name': 'viewport',
...
options2 = []
for product_group in df.product_group_description.unique():
    options2.append({'label': product_group, 'value': product_group})
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col(
                html.H1("""Select Product Group Name""",
        ...
        dbc.Col([
                html.Div(
                    children = dash_table.DataTable(
                        id='product_group_table',
                        # columns =[{'id': c, 'name': c} for c in df.columns],
                        columns=[
                            dict(id='product_group_key', format=Format().align(Align.left)),
                            dict(id='product_group', type='numeric'),
                            dict(id='product_group_description'),
                            dict(id='product'),
...
@app.callback(
    Output('product_group_table', 'data'),
    [Input('product_group_table','active_cell')
...
def update_table(interval, start_date, end_date, value):
    dim_product_group_df = Dim_product_group.objects.all()
    dim_product_group_df = read_frame(dim_product_group_df)
    df = dim_product_group_df

    if value:
        df = df[df['product_group_description'].isin(value)]
    else:
        pass
