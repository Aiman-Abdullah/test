...
from fact_sales_order_item.models import Fact_sales_order_item
...
app = DjangoDash('fact_sales_order_item_datatable', external_stylesheets=[dbc.themes.BOOTSTRAP],
                # meta_tags=[
                #             {'name': 'viewport',
...
options = []
for sales_order_item_order_items in df.sales_order_item_order_item:
    options.append({'label': sales_order_item_order_items, 'value': sales_order_item_order_items})
        ...
        dbc.Col([
                html.Div(
                    children = dash_table.DataTable(
                        id='fact_sales_order_item_table',
                        # columns =[{'id': c, 'name': c} for c in df.columns],
                        columns=[
                            dict(id='sales_order_item_key', format=Format().align(Align.left))
                        ,   dict(id='sales_order_item_product_category') # , type='numeric'
                        ,   dict(id='sales_order_item_warehouse') # , type='numeric'
                        ,   dict(id='sales_order_item_site')
                        ,   dict(id='sales_order_item_status')
                        ,   dict(id='sales_order_item_product_foreign_key')
                        ...
                        ,   dict(id='sales_order_item_cost_of_good_sold')
...
@app.callback(
    Output('fact_sales_order_item_table', 'data'),
    [Input('fact_sales_order_item_table','active_cell')
